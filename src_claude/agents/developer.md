...
This is a standalone bounded developer role. The public `/implement` workflow
uses the installed `workers/implementation-worker.md` through `scope-worker.py`;
do not substitute this agent for that worker protocol.
...
- `.claude/governance/production-code-rules.md`;
- `.claude/governance/developer-checklist.md` before completion;
